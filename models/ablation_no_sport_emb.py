...
class LactateDataset(Dataset):
    ...
    def load_file(self, file_path):
        ...
        h      = float(meta.get("height", 0) or 0) / 200.0   # [FIX A4/F1]
...
class PhysioTransformerNoEmb(nn.Module):
    def __init__(
        self,
        input_dim:  int,
        n_sports:   int = 5,     # kept for API compatibility, unused
        d_model:    int = 128,   # [FIX A2] same as full model
        n_heads:    int = 8,
        n_layers:   int = 4,
        dim_ff:     int = 1024,
        dropout:    float = 0.15,
        ...
        # [FIX A1] No sport embedding — input projection takes raw features only
    ...
    def forward(self, x, L, sport):
        ...
        z    = self.pos(self.inp(x))               # raw features only
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(
        ...
        model = PhysioTransformerNoEmb(input_dim).to(DEVICE)
            ...
            if val_mae < best_val_mae:
                ...
                torch.save(
                    {"state_dict": model.state_dict(),
                     ...
                     "val_mae": val_mae},
                    f"checkpoint_no_emb_fold{fold + 1}.pt",
                )
                print(f"  ✓  Best checkpoint saved  (MAE={val_mae:.2f})")

        ckpt = torch.load(f"checkpoint_no_emb_fold{fold + 1}.pt",
                          map_location=DEVICE)
    ...
    print(f"\n{'='*60}")
    print("5-FOLD GROUPKFOLD RESULTS  —  Ablation A: No Sport Embedding")
    print(f"{'='*60}")
