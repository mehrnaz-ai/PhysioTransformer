"""
model_full.py — PhysioTransformerFull  (Reference / Full Model)

FIXES vs. finall.py:
  [F1] Height normalisation: / 200.0  (was / 2.2, a lbs-to-kg factor — wrong units)
  [F2] hrmax computed ONCE from sheet3 (or np.max(hr) fallback) and used
       consistently for lt_norm, hr_norm, hr_reserve, and the stored hrmax tensor.
       The original code overwrote hrmax with np.max(hr) AFTER computing lt_norm,
       creating a silent inconsistency in the bpm recovery step.
  [F3] Removed three duplicate hr_rest / hr_reserve computation blocks.
  [F4] Removed unused variables: d2hr, cum_hr, remaining_capacity.
  [F5] Uncertainty head properly trained via Gaussian NLL loss.
       Previously logvar was defined and returned but never entered loss_fn.
  [F6] forward() returns logvar (not std) so loss_fn can compute NLL.
       evaluate() converts logvar → std locally for reporting.
  [F7] Best checkpoint saved on VALIDATION MAE, not training loss.
       The original saved whenever training loss dropped (inside the batch loop),
       which could save an overfitted mid-epoch snapshot.
  [F8] Reproducibility: torch + numpy seeds set at top.
  [F9] collate_fn uses torch.stack() instead of torch.tensor() on tensor tuples.
"""
...
class LactateDataset(Dataset):
    ...
    def load_file(self, file_path):
        ...
        # [FIX F3, F4] feature engineering — single clean computation block
        ...
        y = clean(np.log1p(lac).reshape(-1, 1))      # shape: (T, 1)
...
# ================================================================
# MODEL — Full
#   Sport embedding + shared ordinal LT head + uncertainty (NLL-trained)
#   d_model = 128  (fixed reference for all ablations)
# ================================================================
class PhysioTransformerFull(nn.Module):
    def __init__(
        ...
        # [FIX F5, F6] Uncertainty logvar head — trained via NLL in loss_fn
    ...
    def forward(self, x, L, sport):
        ...
        h    = self.repr(h)                         # (B, T, 64)

        # Attention pooling → global vector
        score    = self.attn_pool(h).masked_fill(mask.unsqueeze(-1), -1e9)
        w        = torch.softmax(score, dim=1)       # (B, T, 1)
        h_global = (w * h).sum(dim=1)               # (B, 64)

        # Curve prediction (per step)
        curve = self.curve_head(h)                  # (B, T, 1)

        # LT prediction via expected bin value
        lt_logits = self.lt_bins(h_global)
        lt_prob   = torch.softmax(lt_logits, dim=-1)
        lt_pred   = (lt_prob * self.lt_bin_centers).sum(dim=-1)   # (B,)

        # [FIX F6] Return logvar so loss_fn can compute NLL
        logvar = self.logvar(h_global).clamp(-5.0, 2.0)           # (B, 1)

        return curve, lt_pred, logvar


# ================================================================
# LOSS — [FIX F5] Gaussian NLL properly trains the logvar head
# ================================================================
...
def evaluate(model, loader):
    ...
    for X, y, L, lt, hrmax, sport, _ in loader:
        ...
        # [FIX F6] convert logvar → std for reporting
...
if __name__ == "__main__":
    ...
    input_dim = ds[0][0].shape[1]          # 14 features
